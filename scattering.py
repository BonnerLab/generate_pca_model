...
class Scattering2D(nn.Module):
    """ Implements convolution with complex waveelt filters. """
    def __init__(self, input_type: SplitTensorType, L, scales_per_octave=1, full_angles=False,
        ...
        :param factorize_filters: if True, use filters adapted for two scattering blocks per scale instead of one
        :param i: index of layer in the current scale (see factorize_filters)
        """
        super().__init__()
    ...
    def forward(self, x: SplitTensor) -> Dict[str, SplitTensor]:
        ...
        return phase_scattering2d_batch(x_split=x, pad=self.pad, phi=self.phis, psi=self.psis, subsample=self.subsample)


def phase_scattering2d_batch(x_split: SplitTensor, pad, phi, psi, subsample) -> Dict[str, SplitTensor]:
    """
    ...
    def apply_filter(x, filters, cast_to_real=False):
        ...
        x = channel_reshape(x, (-1, 1))  # (C, 1)
        filters = channel_reshape(filters[None], (1, -1))  # (1, K)
        y = x * filters  # (C, K)
        y = channel_reshape(y, (-1,))  # (CK,)

        if subsample:
            y = subsample_fourier(y, 2)  # (B, CK, M//2, N//2)
        y = torch.fft.ifft2(y)  # (B, CK, M(//2), N(//2))
        if cast_to_real:
            y = y.real  # Should be real anyway.
        y = unpad(y, subsample)  # (B, CK, M(//2)-2(4), N(//2)-2(4))
        return y
    ...
    if psi.shape[0] > 0:
        x_psi = apply_filter(U_0_c, psi)  # (CSL,), complex
...
class ScatNonLinearity(nn.Module):
    ...
    def __init__(self, input_type: SplitTensorType, non_linearity, gain, bias, learned_params):
        """
        :param input_type:
    ...
    def extra_repr(self) -> str:
        ...
        return f"non_linearity={non_lin}, complex={complex}"
    ...
    def forward(self, x: SplitTensor) -> SplitTensor:
        ...
        return SplitTensor(x_abs, groups=x.num_channels)
